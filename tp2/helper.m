classdef Helper
    methods(Static)
        ...
        function res = touchesPost(ri) 
           ...
           res = getDistance(posts(1).x, ri.x, posts(1).y, ri.y) <= Constants.BALL_RADIUS || getDistance(posts(2).x, ri.x, posts(2).y, ri.y) <= Constants.BALL_RADIUS || touchesHorizontalPost(ri);
        end
        ...
        function res = isGoal(ri)
           posts = getPosts(ri);
           if (ri.z >= Constants.GOAL_HEIGHT - Constants.BALL_RADIUS || ri.y <= posts(1).y + Constants.BALL_RADIUS || ri.y >= posts(2).y - Constants.BALL_RADIUS)
              res = false;
              return;
           end

           res = ri.x <= Constants.MIN_X || ri.x >= Constants.MAX_X;
        end

        % Returns whether or not the ball is out the sides of the pitch.
        function res = isFreeThrow(ri)
           res = ri.y <= Constants.MIN_Y - Constants.BALL_RADIUS || ri.y >= Constants.MAX_Y + Constants.BALL_RADIUS;
        end

        % Returns whether or not the ball is behind the goal line without behing a goal.
        function res = isGoalKick(ri)
           res = ~isGoal(ri) && ri.x <= Constants.MIN_X - Constants.BALL_RADIUS || ri.x >= Constants.MAX_X + Constants.BALL_RADIUS;
        end
